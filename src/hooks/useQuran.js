...
const useQuran = () => {
    ...
    const loadLastBookmark = async() => {
        try {
            ...
            if (!response.ok) {
                throw new Error('Failed to fetch last bookmark');
            }
            ...
            if (data && data.surah && data.ayah) {
                ...
                // If there's no current content loaded, load the bookmark
                if (quranContent.length === 0) {
                    fetchAyat(data.surah, data.ayah);
                    setSelectedSurah(data.surah);
                    setSelectedAyat(data.ayah);
                }
            }
        } catch (error) {
            console.error('Error loading last bookmark:', error);
        }
    };
